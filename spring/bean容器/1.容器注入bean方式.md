...
public class ApplicationAnnotation {
    ...
    public static void main(String[] args) {
        ApplicationContext ac = new AnnotationConfigApplicationContext(AppScan.class);
    }
}
...
注意，该种方式下，需被扫描加载注入进容器的类仍要增加@Component及其派生注解，对类的侵入依然存在。  

#### 2.2 @ImportResrouce读取已有的配置类
对于已有的使用配置文件进行bean注入容器的项目，一般不会手动修改对应类逐一增加@Component注解，spring提供了@ImportResource读取已经存在的xml配置：
```java
/*
* 容器启动类
*/
...
### 3.干预bean注入
此部分仅作基础的说明，该小节设计的重要接口将会在后续文档中进行详细说明。
#### 3.1 工厂bean
